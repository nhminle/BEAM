...
def clean_book_name(filename):
    """
    Remove known suffixes from the filename to extract the book name.
    """
    if filename.endswith("_unmasked_passages.csv"):
        return filename[:-len("_unmasked_passages.csv")].replace("_"," ")
    elif filename.endswith("_non_NE.csv"):
        return filename[:-len("_non_NE.csv")]
    else:
...
# ----------------------------------------------------------------
# EXAMPLE USAGE: Change the following paths to match your system.
# ----------------------------------------------------------------
occurrence_csv = '/home/ekorukluoglu_umass_edu/beam2/BEAM/scripts/Evaluation/dir_probe/Percentage_of_Distinct_Passages_per_Filename.csv'
accuracy_csv   = '/home/ekorukluoglu_umass_edu/beam2/BEAM/aggregated_en_accuracies.csv'
save_path      = '/home/ekorukluoglu_umass_edu/beam2/'
