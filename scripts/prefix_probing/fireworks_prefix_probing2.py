    ...
    for retry in range(retries):
        try:
            ...
            if isinstance(prompt, str):
                ...
                raise Exception("prompt type not str")
        except Exception as e:
            print("Some error happened with api, retrying..." + str(e))
            time.sleep(5)
            if not retry<retries-1:
...
def prefixProbe(csv_file_name, book_title, prompt_setting="zero-shot"):
    try:
        ...
        df_out.to_csv(f"./prefix_probing/fireworks_out/{book_title}_prefix_probe_llama405b.csv", index=False, encoding='utf-8')
    except Exception as e:
        print(e)
        print(f"error occured while running column {lang}")
