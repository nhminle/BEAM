...
def eval_guess(correct, guess):
    if "Huckleberry Finn" in correct and "Huckleberry Finn" in guess:
        return "Correct"
...
def direct_probe(csv_file_name, book_title, book_author):
    try:
        ...
        df.to_csv(f"/direct_probing/out/{title.replace(' ', '_')}_direct-probe_olmo", index=False, encoding='utf-8')
    except:
