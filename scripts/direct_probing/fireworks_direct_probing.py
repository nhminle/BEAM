...
def direct_probe(csv_file_name, book_title, prompt_setting,mode):
    try:
        ...
        for language in df.columns:
            ...
            for i in range(len(df)):
                ...
                try:
                    ...
                    content = predict(base_language, passage, mode, prompt_setting,mode)
...
if __name__ == "__main__":
    ...
    for title in titles:
        ...
        # logging.info("done")
        direct_probe(csv_file_name=f"./Prompts/{title}/{title}_filtered_sampled.csv", book_title=title, prompt_setting="0s",mode="NE") # modify the prompt setting here
        direct_probe(csv_file_name=f"./Prompts/{title}/{title}_filtered_sampled.csv", book_title=title, prompt_setting="1s",mode="NE") # modify the prompt setting here      
